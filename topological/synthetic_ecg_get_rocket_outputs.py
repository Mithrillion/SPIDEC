...
output_file = Path("outputs/synthetic_ecg_rocket.csv")
...
for id in tqdm(range(200)):
    ...
    rocket = Rocket(num_kernels=50, normalise=False, n_jobs=-1, random_state=7777)
    ...
    clusterer = KMeans(n_clusters=k, random_state=7777)
    ...
    hmg, cpl, vm = homogeneity_completeness_v_measure(gt_labels, clusterer.labels_)
    sil = silhouette_score(trace_subseqs, clusterer.labels_)
    scores = pd.DataFrame(
        [
            pd.Series(
                {
    ...
    hv.save(g3, f"plots/synthetic_ecg_rocket/{id}_segment.png")
